...
/** Cor por passo — deixa o fluxo legível de relance. */
const STEP_TONE = {
  1: { bg: "rgba(124,92,255,0.18)", ink: "#a995ff" },
  2: { bg: "rgba(45,199,232,0.18)", ink: "#67dcf5" },
  3: { bg: "rgba(18,183,106,0.18)", ink: "#6ce9a6" },
  4: { bg: "rgba(255,176,32,0.18)", ink: "#ffc861" },
  5: { bg: "rgba(255,107,69,0.20)", ink: "#ff9878" },
} as const;
...
function StepTitle({
  ...
  return (
    <div className="mb-3.5">
      <h2 className="flex items-center gap-2.5 text-sm font-semibold tracking-[-0.01em] text-[var(--ink)]">
        <span
          className="flex h-[22px] w-[22px] items-center justify-center rounded-[7px] text-xs font-bold"
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px]">
      ...
      <div className="flex flex-col items-start gap-5 xl:flex-row">
        {/* Coluna de configuração — fica sempre à vista */}
        <div className="flex w-full shrink-0 flex-col gap-4 xl:sticky xl:top-6 xl:w-[320px]">
          <Panel>
            <StepTitle step={1}>O que gerar</StepTitle>
            <div className="flex flex-wrap gap-2">
              {(
                ...
                <button
                  ...
                  className={`flex-1 whitespace-nowrap rounded-xl px-3 py-2 text-sm font-semibold ${
                    kind === id
          ...
          <Panel>
            <StepTitle step={2}>Personagem</StepTitle>
            {loading ? (
          ...
          <Panel>
            <StepTitle step={5}>Enviar para Claude</StepTitle>
            ...
            <Button
              className="w-full !bg-[var(--accent-2)] shadow-[0_8px_20px_rgba(255,107,69,0.22)] hover:!brightness-110"
              loading={busy}
        ...
        {/* Coluna de conteúdo — looks, cenário e prompt */}
        <div className="flex w-full min-w-0 flex-1 flex-col gap-4">
          <Panel>
            <StepTitle
              step={3}
              hint="Escolha o look — toque no card (Peça ou Vestida). A lupa só amplia."
            >
              Look
            </StepTitle>
            {loading ? (
              ...
              <>
                ...
                {otherOutfits.length ? (
                  <>
                    <p className="mb-2 mt-5 text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--muted)]">
                      Outros looks
                    </p>
                    <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4">
                      {otherOutfits.map((o) => (
                    ...
                    {FRAMING_OPTIONS.map((f) => (
                      <button
                        ...
                        className={`rounded-xl border px-2 py-2 text-center text-[11px] font-medium ${
                          framing === f.id
                            ? "border-[var(--ok)] bg-[var(--success-bg)] text-[var(--success-text)]"
            ...
            <p className="mb-2 mt-5 text-[13px] font-medium text-[var(--ink)]">Cenário</p>
            <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4">
              <button
                type="button"
                onClick={(e) => {
                ...
                className={`overflow-hidden rounded-xl border text-left ${
                  keepSceneFromPhoto
                    ? "border-[var(--ok)] ring-2 ring-[var(--success-bg)]"
